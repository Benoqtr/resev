...
# ASCII Logo
$Logo = @"
______  _____  _____  _______   _________     ______  ____  ____  ____    ____  
|_   _ \|_   _||_   _||_   __ \ |  _   _  |  .' ___  ||_  _||_  _||_   \  /   _| 
  | |_) | | |    | |    | |__) ||_/ | | \_| / .'   \_|  \ \  / /    |   \/   |   
  |  __'. | '    ' |    |  ___/     | |     | |   ____   \ \/ /     | |\  /| |   
 _| |__) | \ \__/ /    _| |_       _| |_    \ `.___]  |  _|  |_    _| |_\/_| |_  
 |_______/   `.__.'    |_____|     |_____|    `._____.'  |______|  |_____||_____| 
...
function Get-LatestVersion {
    try {
        $latestRelease = Invoke-RestMethod -Uri "https://api.github.com/repos/Benoqtr/resev/releases/latest"
        return @{
...
Write-Host "Created by Benoqtr`n" -ForegroundColor $Theme.Info

# Set TLS 1.2
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

# Main installation function
function Install-FASTRESERV {
    Write-Styled "Start downloading FAST RESERV" -Color $Theme.Primary -Prefix "Download"
    ...
    try {
        ...
        $asset = $releaseInfo.Assets | Where-Object { $_.name -eq "fast_reservation.exe" }
        if (!$asset) {
            Write-Styled "File not found: fast_reservation.exe" -Color $Theme.Error -Prefix "Error"
        ...
        $downloadPath = Join-Path $DownloadsPath "fast_reservation.exe"
